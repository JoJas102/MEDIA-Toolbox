...
def nnlsfit(A, H, Lambda, Decay):
    s = nnls( [[A][(Lambda)*H]].T*[[A][(Lambda)*H]], [[A][(Lambda)*H]].T*[[ Decay][np.zeros((len(H[:][1]),1))]])
    return s


def getG(A, H, I, Lambda, Decay):
    NNLSfit = nnlsfit( A, H, Lambda, Decay )
    G = norm( Decay - A*NNLSfit )^2/np.trace( I - A*( A.T*A + (Lambda)*H.T*H )^-1*A.T )^2
    return G
...
# regularised NNLS fitting based on CVNNLS.m of the AnalyzeNNLS by Bjarnason et al.
    ...
    H = -2*np.identity(Dlength, Dlength) + np.diagonal(np.ones(Dlength-1,1),1) + np.diagonal(np.ones(Dlength-1,1),-1)
